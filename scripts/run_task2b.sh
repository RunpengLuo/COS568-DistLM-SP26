#!/bin/bash
# Task 2b: Distributed training with all_reduce (4 nodes, 1 epoch)
# Run locally — SSHes into all 4 CloudLab nodes in parallel.
# Usage: bash scripts/run_task2b.sh
...
echo "=== Launching Task 2b (all_reduce) on 4 nodes ==="
for i in 0 1 2 3; do
  ssh $SSH_OPTS "${NODES[$i]}" "cd ~ && export PATH=\$HOME/.local/bin:\$PATH && python3 run_glue.py \
    ...
    --world_size 4 \
    --local_rank $i \
    --comm_method all_reduce 2>&1 | tee /tmp/RTE/rank_\${i}.log" 2>&1 | sed "s/^/[Rank $i] /" &
done
wait

# Fetch results from rank 0
RESULTS_DIR="$SCRIPT_DIR/results/task2b"
mkdir -p "$RESULTS_DIR"
